...
def linear_model_g(X_numpy,y_numpy,weights,learning_rate,num_epoch):
  ...
  weights = torch.from_numpy(numpy.array(weights).astype(numpy.float32))
...
load_path = f"/opt/mlfta/fanitabasib0/Lab/Mahnaz/lstm_95811v_300emb_128hid_4nc_bidirectional_1lay_basic_english_70e_1e-05lr.pt"
model = SimpleBidirectionalLSTM(
    vocab_size=len(vocab),
    embedding_dim=300,
    hidden_dim=128,
...
# Prediction F
def predict_lstm(text, text_transform, model, token_ids):
...
def sampling_token_space(tokens):
...
def binary_vector_creator(corpus, doc_index, text_transform):
    ...
    return embedding_x, tokens, token_ids_x
...
def corpus_modifier(percent, exp_file, corpus_test, text_transform):
  ...
  for index, explanation in enumerate(exp_file):
    ...
    binary_vector, tokens_x, token_ids_x = binary_vector_creator(corpus_test, index, text_transform)
...
#for ind in range(10):
  ...
  for i in range(num_puturb):
    try:
      selected_tokens, indices = sampling_token_space(tokens_x)
      z_1 = torch.tensor([0 if token in selected_tokens else 1 for token in tokens_x], dtype=torch.float32)
      z = [element.item() if index not in indices else 0 for index, element in enumerate(token_ids_x)]
      # Prediction
      label_f = predict_lstm(text = None, text_transform=text_transform, model=model, token_ids=z)
      # compute weigths
  ...
  # Train g
  final_weights = linear_model_g(numpy.array(lis_z_1), numpy.array(lis_f), numpy.array(lis_w), learning_rate=0.1,num_epoch=10)
  # Get the explanation
  explanation_implemented_lime = dict(zip(numpy.array(tokens_x),final_weights[0]))
  explanation_implemented_lime = dict(sorted(explanation_implemented_lime.items(), key=lambda x: x[1], reverse=True))

  try:
    with open('exp_file_token.txt', 'a') as f:
      f.write(str(explanation_implemented_lime) + '\n')
    f.close()
  except:
    with open('exp_file_token.txt', 'w') as f:
      f.write(str(explanation_implemented_lime) + '\n')
    f.close()


with open('exp_file_token.txt', 'r') as f:
